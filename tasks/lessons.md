...
- Before pinning an installer to an older release, compare that archive's runtime layout with the current install contract. A legacy archive can be checksum-valid yet correctly lack newer bundled runtimes, so its compatibility contract must be explicit rather than treated as a corrupt download.
- When a user reports behavior after a standalone install, resolve the exact command with `which -a` before changing runtime code. Version-manager shims can shadow the installed binary and make every release fix appear ineffective.
