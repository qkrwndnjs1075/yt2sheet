...
import { z } from "zod";
import { corpusManifestSchema } from "../benchmarks/corpus-schema";
import { BenchmarkRunError, canonicalJson } from "../benchmarks/runner";
...
test("returns the strict usage error when the benchmark CLI receives an unknown option", () => {
  ...
  const result = spawnSync(process.execPath, ["--import", "tsx", "benchmarks/cli.ts", "run", "--unknown"], {
    cwd: process.cwd(),
    encoding: "utf8",
  });
...
test("writes byte-identical stable mechanics artifacts under two output roots", async (t) => {
  ...
  assert.match(runText, /"relativePath":"pipeline\/youtube-score-processor\.ts"/);
  const youtubeSource = await readFile(resolve("pipeline/youtube-score-processor.ts"));
  const youtubeSha256 = createHash("sha256").update(youtubeSource).digest("hex");
  assert.match(runText, new RegExp(`"relativePath":"pipeline/youtube-score-processor\\.ts","sha256":"${youtubeSha256}"`));
  const productionModules = (await readdir(resolve("pipeline"), { recursive: true })).filter((path) =>
...
function runCli(scenario: CliScenario) {
  ...
  return spawnSync(process.execPath, ["--import", "tsx", "benchmarks/cli.ts", "run", "--manifest", scenario.manifestPath,
