...
import { resolveScoreTimeRange } from "../server/score-time-range";
import { ScoreJobService, type ScoreJobInput } from "../server/score-job-service";

describe("score time range resolver", () => {
  it("accepts the existing no-range input through the score job service", async () => {
    // Given
    const input = {
      videoId: "abcdefghijk",
      videoUrl: "https://youtu.be/abcdefghijk"
    } satisfies ScoreJobInput;
    const service = new ScoreJobService({
      process: async () => {
        throw new Error("processor must not run for a cancelled job");
      }
    }, { createId: () => "no-range-job" });

    // When
    const job = service.create(input);
    const cancellation = service.cancel(job.jobId);
