...
import { createScorePdfFromFrames } from "../server/score-video-processor.js";

const CREATED_AT = new Date("2026-07-24T00:00:00.000Z");
const VIDEO_ID = "AbCdEfG1234";
const EXPECTED_PDF = { pageWidth: 1200, pageHeight: 1700, rasterWidth: 1200, rasterHeight: 1700, targetDpi: 72 } as const;
...
test("standalone PDF renderer writes the established single layout with privacy-minimal metadata", async (t) => {
  ...
  const outputPath = join(directory, "result.pdf");
    ...
    assertClose(page.getWidth(), EXPECTED_PDF.pageWidth);
    assertClose(page.getHeight(), EXPECTED_PDF.pageHeight);
    ...
    assert.equal(document.getProducer(), "yt2sheet 0.1.0");
...
async function writeScoreFrame(path: string): Promise<void> {
